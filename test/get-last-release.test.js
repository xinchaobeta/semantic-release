...
test('Get the highest non-prerelease valid tag', async t => {
  ...
  const result = await getLastRelease({tagFormat: `v\${version}`, cwd}, t.context.logger);
...
test('Match the tag name from the begining of the string', async t => {
  ...
  const result = await getLastRelease({tagFormat: `prefix/v\${version}`, cwd}, t.context.logger);
...
test('Get the highest valid tag corresponding to the "tagFormat"', async t => {
  ...
  t.deepEqual(await getLastRelease({tagFormat: `\${version}`, cwd}, t.context.logger), {
    gitHead,
    gitTag: '1.0.0',
    version: '1.0.0',
  });
  ...
  t.deepEqual(await getLastRelease({tagFormat: `foo-\${version}-bar`, cwd}, t.context.logger), {
    gitHead,
  ...
  t.deepEqual(await getLastRelease({tagFormat: `foo-v\${version}-bar`, cwd}, t.context.logger), {
    gitHead,
  ...
  t.deepEqual(await getLastRelease({tagFormat: `(.+)/\${version}/(a-z)`, cwd}, t.context.logger), {
    gitHead,
    gitTag: '(.+)/1.0.0/(a-z)',
    version: '1.0.0',
  });
  ...
  t.deepEqual(await getLastRelease({tagFormat: `2.0.0-\${version}-bar.1`, cwd}, t.context.logger), {
    gitHead,
  ...
  t.deepEqual(await getLastRelease({tagFormat: `\${version}-bar.1`, cwd}, t.context.logger), {
    gitHead,
    gitTag: '3.0.0-bar.1',
    version: '3.0.0',
  });
