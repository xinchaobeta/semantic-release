...
test('Get all commits when there is no last release', async t => {
  ...
  const result = await getCommits(undefined, 'master', t.context.logger, {cwd});
...
test('Get all commits since gitHead (from lastRelease)', async t => {
  ...
  const result = await getCommits(commits[commits.length - 1].hash, 'master', t.context.logger, {cwd});
...
test('Return empty array if lastRelease.gitHead is the last commit', async t => {
  ...
  const result = await getCommits(commits[0].hash, 'master', t.context.logger, {cwd});
