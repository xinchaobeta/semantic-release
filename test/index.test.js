...
const stdout = stub(process.stdout, 'write');
...
test('Plugins are called with expected values', async t => {
  ...
  const config = {cwd, branch: 'master', repositoryUrl, globalOpt: 'global', tagFormat: `v\${version}`};
  const options = {
    ...config,
  ...
  t.truthy(await semanticRelease(options, env));
...
test('Use custom tag format', async t => {
  ...
  const notes = 'Release notes';
  const config = {cwd, branch: 'master', repositoryUrl, globalOpt: 'global', tagFormat: `test-\${version}`};
  const options = {
    ...config,
...
test('Log all "verifyConditions" errors', async t => {
  ...
  const config = {cwd, branch: 'master', repositoryUrl, tagFormat: `v\${version}`};
  const options = {
    ...config,
...
test('Returns falsy value if triggered by a PR', async t => {
  ...
  t.falsy(await semanticRelease({cwd, repositoryUrl}, {}));
  t.is(
...
test('Exclude commits with [skip release] or [release skip] from analysis', async t => {
  ...
  await gitPush(repositoryUrl, 'master', {cwd});
  const analyzeCommits = stub().resolves();
  const config = {cwd, branch: 'master', repositoryUrl, globalOpt: 'global'};
  const options = {
    ...config,
