...
test.serial('Return versions merged from release to lts branch', t => {
  const result = getReleasesToAdd(
    ...
    {tagFormat: `v\${version}`, branches: [{name: '1.x', channel: '1.x'}, {name: 'master'}]},
    t.context.logger
  );
...
test.serial('Return versions merged from future branch to release branch', t => {
  const result = getReleasesToAdd(
    {
      name: 'master',
      tags: [
...
test.serial('Return releases sorted by ascending order', t => {
  const result = getReleasesToAdd(
    {
      name: 'master',
      tags: [
...
test.serial('no lastRelease', t => {
  const result = getReleasesToAdd(
    {name: 'master', tags: [{gitTag: 'v1.0.0@next', version: '1.0.0', channel: 'next', gitHead: '111'}]},
    {tagFormat: `v\${version}`, branches: [{name: 'master'}, {name: 'next', channel: 'next'}]},
    t.context.logger
  );
...
test.serial('Ignore pre-release versions', t => {
  const result = getReleasesToAdd(
    {
      name: 'master',
      tags: [
