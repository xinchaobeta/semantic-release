 ...
 */
module.exports = async ({tagFormat, cwd}, logger) => {
  ...
  const tag = await pLocate(tags, tag => isRefInHistory(tag.gitTag, {cwd}), {preserveOrder: true});

  if (tag) {
    logger.log('Found git tag %s associated with version %s', tag.gitTag, tag.version);
    return {gitHead: await gitTagHead(tag.gitTag, {cwd}), ...tag};
  }
